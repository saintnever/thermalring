...
int MLX90640_GetFrameData(struct mlx90640_dev *dev, uint16_t *frameData)
{
    ...
    while(dataReady == 0)
    {
        error = MLX90640_I2CRead(dev, 0x8000, 1, &statusRegister);
        if(error != 0)
        {
            return error;
        }    
    ...
    { 
        error = MLX90640_I2CWrite(dev, 0x8000, 0x0030);
        if(error != 0)
        {
            return error;
        }
            
        error = MLX90640_I2CRead(dev, 0x0400, 832, frameData); 
        if(error != 0)
        {
            return error;
        }
    ...
    error = MLX90640_I2CRead(dev, 0x800D, 1, &controlRegister1);
...
int MLX90640_GetFrameData8(struct mlx90640_dev *dev, uint8_t *frameData)
{
    ...
    { 
        ...
        error = MLX90640_I2CRead8(dev, 0x0400, 1664, frameData); 
        if(error != 0)
        {
            return error;
        }
    ...
    error = MLX90640_I2CRead(dev, 0x800D, 1, &controlRegister1);
    frameData[1664] = controlRegister1>>8;
    frameData[1665] = controlRegister1&0xFF;
    frameData[1666] = 0;
    frameData[1667] = statusRegister & 0x01;
...
int MLX90640_SetResolution(struct mlx90640_dev *dev, uint8_t resolution)
{
    ...
    if(error == 0)
    {
        ...
        error = MLX90640_I2CWrite(dev, 0x800D, value);        
    }    
